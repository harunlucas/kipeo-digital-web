...
import { servicesFaq } from "@/content/services-page";

/**
 * Native <details>/<summary> accordion: keyboard- and screen-reader
 * accessible with no JavaScript required, and the exact content also
 * feeds the FAQPage structured data in the page's JSON-LD.
 */
  ...
  return (
    <Section id="faq" tone="elevated">
      <div className="max-w-2xl">
        ...
        <Reveal delay={0.06}>
          <h2 className="text-display-2 mt-4 text-paper-foreground">Frequently asked questions.</h2>
        </Reveal>
      </div>
          ...
          <Reveal key={item.question} delay={Math.min(index * 0.03, 0.24)}>
            <details className="group py-5">
              <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-4 text-left">
                <span className="text-base font-medium text-paper-foreground sm:text-lg">{item.question}</span>
                <Plus
                  className="h-5 w-5 shrink-0 text-teal-strong transition-transform duration-200 group-open:rotate-45"
                  aria-hidden
                />
              </summary>
              <p className="mt-3 max-w-3xl text-sm leading-relaxed text-slate sm:text-base">{item.answer}</p>
            </details>
          </Reveal>
        ))}
      </div>
